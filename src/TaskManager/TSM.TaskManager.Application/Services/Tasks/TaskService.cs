...
using TaskEntity = TSM.TaskManager.Domain.Entities.Task;
...
internal sealed class TaskService : ITaskService
{
	...
	public async Task<PagedList<SearchTaskResponse>> Search(SearchTasksRequest request, CancellationToken cancellationToken)
	{
		...
		await Task.WhenAll(countTask, chankTask);
	...
	public async Task Delete(DeleteTaskRequest request, CancellationToken cancellationToken)
	{
		var task = await _tasksSet
			.Where(task => task.Id == request.Id)
			.FirstOrDefaultAsync(cancellationToken);
